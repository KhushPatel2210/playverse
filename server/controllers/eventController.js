...
exports.getAllEvents = async (req, res) => {
  try {
    const { sport, page, limit, pastEvent } = req.query;
    let filter = {};
    ...
    const filteredEvents = events.filter((event) => {
      if (pastEvent === "true") {
        // For past events, only show if there is at least one successful participant
        return (
          event.participants &&
          event.participants.some((p) => p.paymentStatus === "success")
        );
      } else {
        // For upcoming events (today, tomorrow, etc), show all events regardless of participants
        return true;
      }
    });

    const eventsWithSlots = filteredEvents.map((event) => {
...
exports.getEventById = async (req, res) => {
  ...
  try {
    ...
    // Use event's mapUrl if available, otherwise search venue
    let mapUrl = event.mapUrl || "";
...
exports.createEvent = async (req, res) => {
  ...
  try {
    ...
    const event = new Event({
      ...
      city,
      sportsName,
      mapUrl: mapUrl || "", // Add mapUrl field
      skillLevel,
      slug: eventSlug,
    });

    const finalSlug = `${eventSlug}-${event._id}`;
...
exports.initiateBooking = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      // Check if it's a valid ObjectId
      if (mongoose.Types.ObjectId.isValid(id)) {
        event = await Event.findById(id).session(session);
      } else {
        // If not a valid ObjectId, assume it's a slug
        event = await Event.findOne({ slug: id }).session(session);
      }
      ...
      const eventDetails = {
        eventId: event.slug || eventId || "", // Use slug if available, fallback to eventId
...
exports.handlePayuSuccess = async (req, res) => {
  try {
    ...
    // Extract event slug from UDF field
    const eventSlug =
    ...
    if (!eventSlug) {
      console.error(
        "Missing or invalid event slug in success redirect. TXN ID:",
        req.body.txnid,
        "UDF Fields:",
        {
          udf1: req.body.udf1,
    ...
    return res.redirect(
      `${process.env.PAYMENT_REDIRECT_URL}/event/${eventSlug}`
    );
  } catch (error) {
    console.error(
...
exports.handlePayuFailure = async (req, res) => {
  try {
    ...
    // Extract event slug if available to redirect back to specific event
    const eventSlug =
      req.body.udf1 && req.body.udf1 !== "undefined"
        ? req.body.udf3.trim()
        : null;

    // Ensure PAYMENT_REDIRECT_URL is defined and does not end with '/'
  ...
  try {
    ...
    const events = data.map((event) => {
      if (
        ...
        throw new Error(
          "Each event must have all required fields, including skillLevel"
        );
      }

      const eventDate = excelSerialToJSDate(event.date);
      ...
      return {
        ...
        actualPrice: Number(event.actualPrice),
        ...
        mapUrl: event.mapUrl || "", // Add mapUrl field from Excel
        slug: "", // temp empty slug
      };
    });

    const insertedEvents = await Event.insertMany(events);

    const updates = insertedEvents.map(async (ev) => {
      const eventSlug = slugifyEvent(
        ev.venueName,
        ev.location,
        ev.date.toISOString(),
        ev.slot
      );
      ev.slug = `${eventSlug}-${ev._id.toHexString()}`;
      return ev.save();
    });
...
exports.sendConfirmation = async (req, res) => {
  ...
  try {
    ...
    const sanitizeImageUrl = (url) => {
      if (!url) return null;
      return url.replace(/\.(jpeg|jpg|png|gif)$/i, "");
    };
    const payload = {
          ...
          to_and_components: participants.map((participant) => ({
            ...
            components: {
              header_1: {
                type: "image",
                value:
                  sanitizeImageUrl(event.venueImage) ||
                  "https://files.msg91.com/432091/vcaifgxt",
              },
...
exports.sendCancellation = async (req, res) => {
  ...
  try {
    ...
    event.cancellationReasons.push(reason); // Push new reason to the array
    await event.save();
...
// Send MSG91 notifications for 75% and 100% booking levels
exports.sendBookingNotifications = async (req, res) => {
  const { id } = req.params;
  const { notificationType } = req.body; // '75' or '100'
  ...
  try {
    ...
    const occupancy = (totalBookedSlots / event.participantsLimit) * 100;

    // Validate notification type
    if (notificationType !== "75" && notificationType !== "100") {
      return res
        .status(400)
        .json({ error: "Invalid notification type. Must be '75' or '100'" });
    }

    // Check if notification should be sent and hasn't been sent already
    const shouldSend75 =
      notificationType === "75" && occupancy >= 75 && !event.notified75;
    const shouldSend100 =
      notificationType === "100" && occupancy >= 100 && !event.notified100;

    if (!shouldSend75 && !shouldSend100) {
      if (notificationType === "75" && event.notified75) {
        return res.status(400).json({
          error: `75% notification already sent for this event`,
        });
      }
      if (notificationType === "100" && event.notified100) {
        return res.status(400).json({
          error: `100% notification already sent for this event`,
        });
      }
      return res.status(400).json({
        error: `Cannot send ${notificationType}% notification. Current occupancy: ${
    ...
    // Use the existing MSG91 utility function
    const { sendWhatsAppMsg91 } = require("../utils/msg91");

    // Map data according to MSG91 template structure:
    // body_1: Event Date
    // body_2: Event Time
    // body_3: Venue Name
    // body_4: Slots filled (75% or 100%)
    // body_5: Customer name (all participant names)
    // body_6: Event Link
    // body_7: Contact no. (all participant phone numbers)
    await sendWhatsAppMsg91(
      ADMIN_PHONE_NUMBER,
      formattedDate, // body_1: Event Date
      event.slot || "", // body_2: Event Time
      event.venueName || "", // body_3: Venue Name
      `${notificationType}%`, // body_4: Slots filled
      allNames, // body_5: Customer name
      `${process.env.CLIENT_BASE_URL}/event/${event.slug || event._id}`, // body_6: Event Link
      allNumbers // body_7: Contact no.
    );

    // Mark notification as sent in the database
    if (notificationType === "75") {
      event.notified75 = true;
    } else if (notificationType === "100") {
      event.notified100 = true;
    }
    await event.save();

    console.log(
      `${notificationType}% notification sent successfully for event: ${event.name}`
    );
    ...
    res
      .status(500)
      .json({ error: `Failed to send ${notificationType}% notification` });
  }
};
...
exports.refundPayment = async (req, res) => {
  ...
  try {
    ...
    console.log("Event ID:", id, "Participant ID:", participantId);
    ...
    if (!participant.paymentId) {
      console.log("No paymentId for this participant");
      return res
        .status(400)
    ...
    const payuUrl = process.env.PAYU_REFUND_URL;
    const mihpayid = participant.paymentId;
    const amount = parseFloat(
      participant.amount?.toString().replace(",", "").trim()
    ).toFixed(2); // refund amount
    ...
    const var2 = token;
    const var3 = amount;
    console.log("mihpayid", var1);
    console.log("token", var2);
    console.log("amount", var3);
    ...
    // --- Update participant as refunded if PayU refund is successful ---
    // You may want to check payuRes.data for actual success, but for now, mark as refunded if no error
    if (payuRes.data && payuRes.data.status === 1) {
      participant.refunded = true;
      participant.refundDate = new Date();
      if (payuRes.data.result && payuRes.data.result.refundid) {
        participant.refundId = payuRes.data.result.refundid;
      }
      await event.save();
    }

    res.status(200).json({ refundResult: payuRes.data, participant });
  } catch (error) {
    console.error(
