...
const createVenue = async (req, res) => {
    try {
        const { name, location, city, sport, imageUrl, description, generalInstructions, mapUrl, amenities, actualPrice } = req.body;
        if (!name || !location || !city || !sport || !imageUrl || !actualPrice) {
            return res.status(400).json({ success: false, message: "Name, location, city, sport, image URL, and actual price are required" });
        }
